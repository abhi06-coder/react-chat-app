...
import { doc, setDoc, getDoc, updateDoc, onSnapshot, deleteDoc, collection, addDoc, getDocs, arrayUnion } from 'firebase/firestore';
...
export default function App() {
     ...
     useEffect(() => {
        ...
        const unsubscribe = onSnapshot(callDocRef, (snapshot) => {
            ...
            if (data?.offer) {
                ...
                if (window.confirm(`Incoming audio call from ${data.callerName}. Answer?`)) {
                    answerCall(callData);
                }
            }
        });
    ...
    // **MODIFIED** handleRoomSelect to add user to members list
    const handleRoomSelect = async (room) => {
        ...
        try {
            // Add the current user to the members list of the room
            await updateDoc(roomDocRef, {
                memberIds: arrayUnion(user.uid)
            });

            // Update user's personal room history
    ...
    // ... all other handler functions (handleLogout, onProfileComplete, etc.) ...
